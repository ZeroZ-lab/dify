...
4. **Graph Engine 调度机制**：分为三篇——
   - 04A：执行循环与就绪判定（见 `04A_engine_run_and_state.md`）
   - 04B：命令通道、暂停与恢复（见 `04B_commands_pause_resume.md`）
   - 04C：并行扩缩容、层与事件、错误策略（见 `04C_workers_layers_errors.md`）
5. **节点实现与事件**：对比常见节点的 `_run()` 流程及 `node_events/`、`graph_events/` 的协同
6. **业务协作与扩展点**：回到 `services/`、`repositories/`，总结持久化、插件与扩展策略
7. **测试与调试**：整理 `tests/unit_tests/core/workflow/` 的夹具、常见断点与排障步骤
...
- `03 Graph 模型与运行态`：提纲完成（见 `03_graph_model_and_runtime.md`）
- `04A/04B/04C Graph Engine 调度机制`：详细完成（见对应文件）
- `05 节点实现与事件`：提纲完成（见 `05_nodes_and_events.md`）
- `06 业务协作与扩展机制`：提纲完成（见 `06_services_and_extensions.md`）
- `07 测试与调试`：提纲完成（见 `07_testing_and_debugging.md`）
